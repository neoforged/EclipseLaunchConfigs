...
project {
    ...
    params {
        ...
        text("github_repository_name", "EclipseLaunchConfigs", label = "The github repository name. Used to connect to it in VCS Roots.", description = "This is the repository slug on github. So for example `EclipseLaunchConfigs` or `MinecraftForge`. It is interpolated into the global VCS Roots.", display = ParameterDisplay.HIDDEN, allowEmpty = false)
        text("env.PUBLISHED_JAVA_ARTIFACT_ID", "EclipseLaunchConfigs", label = "Published artifact id", description = "The maven coordinate artifact id that has been published by this build. Can not be empty.", allowEmpty = false)
        ...
        text("docker_gradle_version", "8.1", label = "Gradle version", description = "The version of Gradle to use during execution of Gradle tasks.", display = ParameterDisplay.HIDDEN, allowEmpty = false)
    }

    features {
        githubIssues {
            id = "EclipseLaunchConfigs__IssueTracker"
            displayName = "neoforged/EclipseLaunchConfigs"
            repositoryURL = "https://github.com/neoforged/EclipseLaunchConfigs"
        }
    }
}
...
object Build : BuildType({
    ...
    id("EclipseLaunchConfigs__Build")
    name = "Build"
...
object BuildSecondaryBranches : BuildType({
    ...
    id("EclipseLaunchConfigs__BuildSecondaryBranches")
...
object PullRequests : BuildType({
    ...
    id("EclipseLaunchConfigs__PullRequests")
    name = "Pull Requests"
